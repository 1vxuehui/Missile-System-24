...
void shoot_init(void)
{
    ...
    // 拨弹盘pid
    static const fp32 Trigger_speed_pid[3] = {10, 0.3, 1000};
		static const fp32 Trigger_angle_pid[3] = {900, 0, 100};
...
static void Motor_Angle_Cal()
{
	...
	trigger_motor.ANGLE_rev.eer = trigger_motor.shoot_motor_measure->ecd - trigger_motor.shoot_motor_measure->last_ecd;
		if(trigger_motor.ANGLE_rev.eer < -4096)									
		{																	
			trigger_motor.ANGLE_rev.eer += 8192;										
		}																	
		else if(trigger_motor.ANGLE_rev.eer > 4096)							
		{																	
			trigger_motor.ANGLE_rev.eer -= 8192;										
		}																	
		trigger_motor.angle_sum += trigger_motor.ANGLE_rev.eer;					
...
static void trigger2_control_loop(Shoot_Motor_t *trigger_move_control_loop)
{
    ...
    trigger_move_control_loop->motor_pid_angle.max_iout = TRIGGER_BULLET_PID_MAX_IOUT;	
		trigger_move_control_loop->given_angle = PID_Calc(&trigger_move_control_loop->motor_pid_angle, trigger_move_control_loop->angle_sum, trigger_move_control_loop->set_angle);
		if(trigger_move_control_loop->set_angle == 0 && fabs(trigger_move_control_loop->set_angle - trigger_move_control_loop->angle_sum) <= 20)
		{
			trigger_move_control_loop->given_angle=0;
		}
