...
void shoot_control_loop(void)
{
	...
	{
		reload_motor.set_angle += 0.1;
		if(reload_motor.reload_angle_ref >= 40*reload_cnt)
		{
...
//					} 
				...
				if (missile_shoot_move.shoot_rc->rc.ch[2] > 400 && turn_spring_flag == 0)
    {
					pull_spring_motor.set_angle += 0.05;	
//					turn_spring_flag = 1;
    }
		else if (missile_shoot_move.shoot_rc->rc.ch[2] < -400 && turn_spring_flag == 0)
    {
					pull_spring_motor.set_angle -= 0.05;		
//					turn_spring_flag = 1;
    }
